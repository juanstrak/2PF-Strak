...
import { InscripcionesActions } from './inscripciones.action';
...
export class InscripcionesEffects {
      ...
      concatMap(
        (action) =>
          this.inscripcionesService.createInscripcion(action.data)
            .pipe(
              map((res) => InscripcionesActions.createInscripcionSucess({ data: res })),
  ...
  deleteInscripcion$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatMap((action) => this.inscripcionesService.deleteInscripcionById(action.id).pipe(
